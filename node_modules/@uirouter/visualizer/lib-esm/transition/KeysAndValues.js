...
var KeyValueRow = /** @class */ (function (_super) {
    ...
    KeyValueRow.prototype.render = function () {
        ...
        var renderValue = function () {
            if (val === undefined)
                return h("span", { className: "uirTranVis_code" }, "undefined");
            if (val === null)
                return h("span", { className: "uirTranVis_code" }, "null");
            if (typeof val === 'string')
                return h("span", { className: "uirTranVis_code" },
                    "\"",
                    maxLength(100, val),
                    "\"");
            if (typeof val === 'number')
                return h("span", { className: "uirTranVis_code" }, val.toString());
            if (typeof val === 'boolean')
                return h("span", { className: "uirTranVis_code" }, val.toString());
            if (Array.isArray(val))
                return h("span", { className: "link", onClick: showModal }, "[Array]");
            if (typeof val === 'object')
                return h("span", { className: "link", onClick: showModal }, "[Object]");
            if (typeof val.toString === 'function')
                return h("span", null, maxLength(100, val.toString()));
        };
        return (h("div", { className: classes.div },
            h("div", { className: classes.key },
                key,
                ":"),
            h("div", { className: classes.val }, renderValue())));
    };
    return KeyValueRow;
}(Component));
export { KeyValueRow };
var KeysAndValues = /** @class */ (function (_super) {
    ...
    KeysAndValues.prototype.render = function () {
        ...
        return (h("div", { className: "uirTranVis_keysAndValues" },
            tuples.map(function (tuple) { return h(KeyValueRow, { key: tuple.key, tuple: tuple, classes: classes, modalTitle: modalTitle }); }),
            showGroupToggle &&
                !!groupedTuples.length && (h("a", { href: "javascript:void(0)", onClick: function () { return _this.setState({ collapseFalsy: !isCollapsed }); }, className: "uirTranVis_keyValue" },
            ...
            (!showGroupToggle || !this.state.collapseFalsy) &&
                groupedTuples.map(function (tuple) { return (h(KeyValueRow, { key: tuple.key, tuple: tuple, classes: classes, modalTitle: modalTitle })); })));
    };
